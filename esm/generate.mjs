...
//   code-input.d.mts: (../code-input.d.ts without the plugin/template namespaces) + ESM
{
    ...
    await codeInputDMts.writeFile("declare const _default: { Plugin: typeof Plugin, Template: typeof Template, CodeInput: typeof CodeInput, registerTemplate: typeof registerTemplate };\nexport default _default;\n");
...
// Templates
{
    ...
    templateNames.forEach(async (templateName) => {
        ...
        // Imports
        await templateDMts.writeFile("import { Template, Plugin } from \"../code-input.mjs\";\n");
        // Code after start and before end of this template, making use of the imported Template, not codeInput.Template
        let copyingCode = false;
        let classSeen = false;
        for await (let line of codeInputDTs.readLines()) {
            ...
            if(copyingCode) {
                if(/^( |\t)*class [A-Za-z]+ /.test(line) && !classSeen) {
                    // Replace only first occurrence
                    line = line.replace("class", "export default class");
                    classSeen = true;
                }
                await templateDMts.writeFile(line.replaceAll("codeInput.Template", "Template").replaceAll("codeInput.Plugin", "Plugin").replaceAll("codeInput.plugins.", "").replaceAll("codeInput.templates.", "")+"\n");
            }
            if(line.includes("ESM-SUPPORT-START-TEMPLATE-"+templateName)) {
                copyingCode = true; // After is code to copy - this line missed out
            }
...
// Plugins
{
    ...
    for(let i = 0; i < pluginNames.length; i++) {
        ...
        // Imports
        await pluginDMts.writeFile("import { Plugin, CodeInput } from \"../code-input.mjs\";\n");
        ...
        for await (let line of codeInputDTs.readLines()) {
            ...
            if(copyingCode) {
                if(/^( |\t)*class [A-Za-z]+ /.test(line) && !functionSeen) {
                    // Replace only first occurrence
                    line = line.replace("class", "export default class");
                    functionSeen = true;
                }
                await pluginDMts.writeFile(line.replaceAll("codeInput.Plugin", "Plugin").replaceAll("codeInput.CodeInput", "CodeInput").replaceAll("codeInput.plugins.", "").replaceAll("codeInput.templates.", "").replace(/^(\s*)namespace ([A-Za-z]+)/, "$1export namespace $2")+"\n");
            }
