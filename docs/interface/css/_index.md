...
* Any `max-width` or `max-height` rules you style `textarea`s with should use a CSS selector that excludes `textarea` elements inside `code-input` elements (for example, `textarea:not(code-input textarea)`).
...
## Highlighting theme requirements

**code-input.js works with most highlighter themes from major syntax highlighting libraries.** However, if you're making your own theme or have problems please note:

* Token elements in highlighter themes must not have CSS properties which make text move around and change its position or size compared to its unhighlighted layout. **Most code highlighting themes have zero problems here.** However, if selecting code in the code-input element looks wrong compared to the highlighted code, you can use CSS like the following, use a different highlighting theme, or get in touch on a [GitHub](https://github.com/WebCoder49/code-input/issues) or [Codeberg](https://codeberg.org/code-input-js/code-input-js/issues) issue:

```css
/* Fixes most broken themes: */
pre code,
pre code * {
...
/* If the theme uses a proportional font
rather than a monospace font, also include
this CSS: */
pre code,
pre code * {
